...
Current status: **LICENSE_SELECTION_PENDING**. Publication is blocked until the user explicitly selects a license, final ChatGPT repository review is completed, and the user explicitly approves publication. Do not choose a license or insert placeholder license text.
...
ZIP contents are exactly `CargoMarketFix.dll`, `CargoMarketFix.ini`, `README.md`, plus `LICENSE` only after the user's explicit license selection. No source, tests, build metadata, logs, debug files, game/SDK data, saves, profiles, mods, research or private paths.
...
10. Commit reviewed source/docs/checksums and push. Complete license selection and final review. Confirm explicit publication and visibility approval before creating the matching tag/Release or uploading anything.
...
Release notes and checksums are in [releases/v1.0.0](releases/v1.0.0/RELEASE_NOTES.md). Assets remain local and unpublished. License selection, final review and explicit publication approval remain required.

When a license is chosen, add the actual approved LICENSE, update any affected README wording, regenerate the ZIP and its checksums/notes, and rescan the final artifacts before approval. These prepared hashes are not valid for a later modified package. Gameplay source is unchanged by this workflow preparation.
