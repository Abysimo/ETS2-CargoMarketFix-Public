...
PUBLIC cmf_refresh_spread_bridge_v161
PUBLIC cmf_refresh_spread_bridge_v161_end
.code
; Exact 1.61.1.0 ABI: RCX=N, RAX=begin, RBX=cursor, RDI=end, RBP=economy.
; RCX becomes bucket length for the owned JRCXZ. Only RBX/RDI are narrowed.
; RFLAGS are dead: body callee SUB / empty-path epilogue ADD precede any use.
; Same static-unwind / fault-retained lease contract as the legacy bridge.
cmf_refresh_spread_bridge_v161 PROC FRAME
    ...
    ; Reproduce the overwritten original CMP/JE before any minute read.
    cmp rax,rdi
    je v161_empty
    mov rax,rcx
...
v161_done:
    ...
    add rsp,88h
    ret
cmf_refresh_spread_bridge_v161 ENDP
cmf_refresh_spread_bridge_v161_end LABEL BYTE
END
